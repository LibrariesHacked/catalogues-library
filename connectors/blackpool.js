...
getLibrariesInternal = async function (service) {
  ...
  try {
    ...
    const select = /\<select id="library-ddl"[\s\S]+?<\/select>/gm.exec(libraries.text)[0];
...
exports.searchByISBN = async function (isbn, service) {
  ...
  try {
    ...
    titleRequest.body.callInfoField.forEach(info => {
      ...
      responseHoldings.availability.push({ library: lib.name, available: copiesAvailable.length, unavailable: copiesUnavailable.length })
    })
  }
  catch(e) {
